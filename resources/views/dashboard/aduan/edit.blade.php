...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Edit Aduan Konten </h1>
</div>
...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/posts/{{ $post->id }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="link" class="form-control @error('url') is-invalid @enderror" id="url" name="url" required value="{{ old('url', $post->url ) }}">
        ...
        <div class="mb-3">
            <label for="fomrFiles" class="form-control">Upload Image</label>
            <input type="hidden" name="buktilama" value="{{ $post->bukti }}">

            @if($post->bukti)
            <img src="{{ asset('storage/' . $post->bukti) }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
            @else
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="keterangan" name="keterangan" rows="3"> {{ @old('keterangan', $post->keterangan) }} </textarea>
        </div>
